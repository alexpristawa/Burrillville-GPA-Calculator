...
let activeGrade = 9;
let info;
let previousLocalStorage = localStorage.gpaCalculator;
if(previousLocalStorage !== undefined) {
    info = JSON.parse(previousLocalStorage).info;
} else {
    info = {
        9: [],
        10: [],
...
document.addEventListener('keydown', (event) => {
    if(event.key == 'Enter') {
        document.querySelector('.addClassHolder').click();
    }
});
...
let howTo = () => {
    ...
    If you are unsure of the weights of your classes, check them in the program of studies.
    `;
